...
export function showErrors(doc) {
  ...
  html.insertAdjacentHTML(head, 'beforeend', /*html*/`
    <style id="tagel-error-styles">
      ...
      #tagel-error #tagel-errors-output a { display: block; }
      *:has(+ .tagel-error1) { outline: darkred 1px dashed; }
      .tagel-error1 { position: absolute; color: darkred; border: darkred 1px dashed; background: white; }
      .tagel-error1:target { background: yellow; scroll-margin-top: 45vh; }
    </style>
  `)
  const { 'tagel-error': tagelErrorsElement } = html.insertAdjacentHTML(body, 'beforeend', /*html*/`
    <div id="tagel-error">
      ...
      <div id="tagel-errors-output"></div>
    </div>
    <script>
  ...
  for (const error of errors) {
    error.errorId = 'error' + Math.round(Math.random() * 10000)
    if (!errorMessageCounter.has(error.expression + error.message ?? '')) {
      ...
      html.insertAdjacentHTML(tagelErrors, 'beforeend', /*html*/`
        <a href="#${error.errorId}">${error.expression} ${error.message ?? ''}</a>
      `.trim() + '\n')
    }
    if (html.adapter.isTextNode(error.el)) {
      if (error.userEntry) {
        ...
        if (error.inBody) {
          html.insertAdjacentHTML(error.el.parentNode, 'afterend', highlightError1(error.expression, error.errorId))
      ...
      if (html.adapter.isElementNode(error.el)) {
        ...
        html.insertAdjacentHTML(error.el, 'afterend', highlightError1(error.expression, error.errorId))
      }
    }
  }
}
...
export function highlightError1(expression, errorId) {
  return /*html*/`<span class="tagel-error1" id="${errorId}"><em>ERROR1</em> missing: ${expression}</span>`
}
