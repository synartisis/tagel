...
export function showErrors(doc) {
  ...
  const tagelErrorsElement = html.insertAdjacentHTML(body, 'beforeend', /*html*/`
    <code id="tagel-error" style="position: fixed; bottom: 0; color: #e7714d; padding: 1rem; background: #000c; border: solid 2px; cursor: pointer; z-index: 99999999;">
      <strong style="display: block; text-decoration: underline; padding-bottom: 4px;">binding errors found!</strong>
    </code>
  `)
  html.insertAdjacentHTML(body, 'beforeend', /*html*/`
    <script>
      document.querySelector('#tagel-error').addEventListener('click', evt => evt.currentTarget.style.display = 'none')
    </script>
  `)  
