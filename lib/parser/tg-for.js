...
export async function tgForElement(el) {
  if (!el.attribs['tg-for']) return
  const context = getContext(el)
  const expression = el.attribs['tg-for']
  const limit = Number.isInteger(el.attribs['tg-for-limit']) ? Number(el.attribs['tg-for-limit']) : -1
  delete el.attribs['tg-for']
  ...
  try {
    ...
    return addError(el, expression, error)
  }
  if (!Array.isArray(value)) {
    return addError(el, expression, null, `[tg-for] expression "${expression}" does not evaluate to array`)
  }
  if (limit !== -1) value = value.slice(0, limit)
  console.log({expression,limit} ,value?.length)
  const itemTemplate = html.cloneElement(el)
  for await (const itemContext of value.values()) {
    const newItem = html.cloneElement(itemTemplate)
    html.insertBefore(newItem, el)
    setContext(newItem, itemContext)
    await tgElementTree(newItem)
 }
  html.detachNode(el)
}