...
export async function tgBindElement(el) {
...
async function contentBinding(el, context) {
  ...
  if (value !== undefined) {
    ...
    if (bindindAttr === 'tg-html') {
      // html.innerHTML(el, String(value ?? ''))
      const fragment = html.parseFragment(String(value ?? ''))
      if (fragment.children) {

        await Promise.all(
  
          fragment.children.map(async child => {
            html.appendChild(el, child)
            if (child.type === 'tag') {

              await tgElementTree(child)
            }
          })
        )
      }
