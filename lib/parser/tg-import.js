...
export async function tgImportElement(el, state) {
  ...
  const fragment = html.parseFragment(partial.content, el.parentNode)
  ...
  if (href) {
    ...
    try {
      ...
      if (error?.code === 'ENOENT') {
        addError(el, href, error, `cannot find ${path.relative('.', partialPath)} referenced by ${path.relative('.', filename)} (${href})`)
      } else {
        throw error
      }
    }
