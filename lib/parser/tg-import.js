...
import { tgElementTree } from './tg-element.js'

/** @type {(root: html.domhandler.Element, filename: string) => Promise<void>} */
export async function tgImportElement(el, filename) {
  if (!(el.name === 'link' && el.attribs['rel'] === 'import' && !!el.attribs['href'])) return
  const partial = await loadPartials(el, filename)
  if (!partial.content) return
  const partialDoc = html.parseFragment(partial.content, el.parent)
  const partialDir = path.dirname(partial.href)
  rewritePartials(partialDoc, partialDir)
  for await (const child of partialDoc.children) {
    html.insertBefore(child, el)
    if (child.type === 'tag' || child.type === 'script' || child.type === 'style') {
      await tgElementTree(child, filename)
    }
  }
  html.detachNode(el)
}
...
export async function tgImport(root, filename) {
  ...
  for (const partial of partials) {
    ...
    console.log('-', partial.href, Buffer.from(partial.content).byteLength)
